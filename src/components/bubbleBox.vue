<template>
  <div class="ui_bubble">
    ...
    <transition
      :name="getTransition"
      v-if="(image && !imgError) || image == undefined"
    >
      <div
        v-if="active"
        ref="bubble"
        class="bubble"
        :class="['is-' + pos, { 'is-customColor': getCustomColor }]"
        :data-pos="pos"
        :style="getStyle"
        v-init
      >
        ...
        <div v-if="!htmlType && !image" class="wrap" :style="getCustomColor">
          ...
          <span
            v-if="content"
            class="content"
            v-html="$options.filters.lengthLimit(content, contentLimit)"
            :style="getCustomColor"
          ></span>
          <span
            v-if="subcontent"
...
export default {
  ...
  computed: {
    getTransition: function () {
      return this.pos.indexOf('T') >= 0
        ? 'fade_posy_margin'
        : 'fade_posy_margin_reverse'
    },
    getStyle: function () {
      ...
      var wid =
        'width : ' +
        (this.$vnode.data.staticStyle && this.$vnode.data.staticStyle.width
      ...
      var mw =
        ...
        this.$vnode.data.staticStyle['max-width']
          ? this.$vnode.data.staticStyle['max-width']
          : this.max_wid + 'px') +
        ';'
    ...
    getArrowPosX: function () {
      if (this.pos.indexOf('L') >= 0) return 'left: ' + this.arrow_pos_x + 'px'
        ...
        return 'right: ' + this.arrow_pos_x + 'px'
      else return ''
    },
    getCustomColor() {
  ...
  watch: {
    ...
    subject: function ($val) {
      if (!$val && !this.htmlType)
        this.title = $(this.$slots.default[0].elm).text()
      else this.title = $val
    },
  ...
  methods: {
    ...
    set_rePos: function () {
      if (this.pos.indexOf('T') >= 0)
        this.pos_top =
          $(this.$slots.default[0].elm).offset().top - $(window).scrollTop() - 4
      else if (this.pos.indexOf('M') >= 0)
        this.pos_top =
          $(this.$slots.default[0].elm).offset().top -
          $(window).scrollTop() -
          4 +
          $(this.$slots.default[0].elm).outerHeight() / 2
      else if (this.pos.indexOf('B') >= 0)
        this.pos_top =
          $(this.$slots.default[0].elm).offset().top -
          $(window).scrollTop() +
          $(this.$slots.default[0].elm).outerHeight() +
          4

      if (this.pos.indexOf('L') >= 0)
        this.pos_left =
          $(this.$slots.default[0].elm).offset().left -
          $(window).scrollLeft() -
          6
      else if (this.pos.indexOf('R') >= 0)
        this.pos_left =
          $(this.$slots.default[0].elm).offset().left +
          $(this.$slots.default[0].elm).outerWidth() -
          $(window).scrollLeft() +
          6
      else if (this.pos.indexOf('C') >= 0)
        this.pos_left =
          $(this.$slots.default[0].elm).offset().left +
          $(this.$slots.default[0].elm).outerWidth() / 2

      if (
        this.$vnode.data.staticStyle &&
        this.$vnode.data.staticClass == 'is-board' &&
        !this.$vnode.data.staticStyle.width
      )
        this.max_wid = $(this.$el).parent().width()
    },
